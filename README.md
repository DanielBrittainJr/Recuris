...
**Evaluation and evolution code for Recuris: a validation-gated Skill Memory
for frozen agents.**

An agent that fails a task the same way twice has a memory problem, not a
capability problem. Recuris separates the two: a **frozen machine** that never
changes between arms, and an **evolved memory** `M = (E, W, ρ, C)` that does.
A meta-agent proposes changes to the memory; a deterministic gate decides
whether they are kept, using paired held-out evidence and nothing else.

That split is physical here. `src/recuris/` is the machine.
`skill_memories/` is the memory, and it is the only thing that differs between
a bare arm and a skill arm.
...
| | Capability | Needs |
|---|---|---|
| **D1** | τ²-Bench retail and airline, bare and with a Skill Memory, on open-weight or frontier models | an OpenAI-compatible endpoint |
| **D2** | the recursive evolution loop on τ²-Bench | the above, plus a coding agent you supply |
| **D3** | SkillFlow, bare and with a Skill Memory, on open-weight models | Docker, an endpoint |
| **D4** | Terminal-Bench 2.1 test-time adaptation | Docker, an endpoint |

---

# Setup

Python 3.11 or 3.12, `git`, and — for D3 and D4 — Docker. Install once:
...
Two environment variables are needed by every benchmark. Put them in a `.env`
at the repository root, or export them:
...
and the reason is not incidental: τ²-Bench scores every episode with an LLM
user simulator and an LLM assertion judge, both pinned to a reference model.
That pinning is part of the evaluation protocol, not an implementation choice,
so an arm whose downstream agent is a local model still calls out for the
simulator and the judge.
...
Other variables are optional and introduced where they are needed:
`RECURIS_META_MODEL` / `RECURIS_META_BASE_URL` / `RECURIS_META_API_KEY` and
`RECURIS_SESSION_LAUNCHER` for D2; `RECURIS_WORKSPACE`,
`RECURIS_EXTERNAL_ROOT`, `TAU2_ROOT`, `TAU2_DATA_DIR`,
`RECURIS_SKILLFLOW_TASKS` and `RECURIS_TB21_TASKS` to relocate directories;
`TAU2_GATE_TERM`, `TAU2_GATE_TERM_WM` and `TAU2_STATUS_BOARD` for the tau2
treatment.

## D1 — τ²-Bench (retail and airline)
...
and upstream states results are not comparable across that boundary; our
numbers are on the corrected tasks. `third_party/tau2/README.md` has the
details.

Then make the benchmark importable and check everything landed:

```bash
uv pip install -e external/tau2-bench     # or: pip install -e external/tau2-bench
...
### Run: the Skill Memory on the reference model
...
The two commands differ in `--agent` and `--skill-memory`, and in nothing else.
That is the design: the machine is identical, the memory is the variable.

### Run: the Skill Memory on an open-weight model
...
### Run: the Skill Memory on a frontier model
...
## D2 — the recursive evolution loop on τ²-Bench
...
`qualify` runs one scoped session and zero simulations. It exists to fail on
the plumbing before you spend benchmark budget, and is worth running whenever
...
Airline is the same with `--domain airline --splits
splits/tau2/airline_from0_v3.json`.
...
including which tasks were excluded and why, which is the only credible
evidence the held-out set was fixed in advance.
...
**Cost.** Days of wall-clock and thousands of model calls. Our retail campaign
at k=4 spent roughly eleven hours on the first two rounds alone.

## D3 — SkillFlow on open-weight models
...
licence**, so not one line of it is redistributed here: the script fetches
their code into `external/` and patches your copy.
...
symptoms in `third_party/harbor/README.md`; on a normal host, stock harbor
works and the patch should not be applied.
...
That writes one config per task family. Run them **one at a time** —
concurrent harbor jobs exhaust the Docker IPv4 address pool, and the failure
...
after seeing scores. `--routing frozen_insample` reproduces the arm on the
model the packages were evolved on and additionally applies six per-family
overrides that **were** selected by reading those families' own scores. Those
six are in-sample; the renderer prints a warning when you select it.

## D4 — Terminal-Bench 2.1 test-time adaptation
...
**Results depend on your container images, not only on the tasks.** We measured
34.5% versus 40.0% for the same baseline agent on two hosts differing only in
their image snapshot, which is larger than most effects anyone would want to
report. The stratification in `splits/tb21/tta_taskset_v3.json` was derived on
one snapshot, recorded as a hard precondition in
...
attempts against a control at one measures the attempts, not the adaptation,
and most of the apparent gain is exactly that.
...
Every number below is a **paired** contrast: the same tasks, the same trials,
the same model, differing only in whether the Skill Memory is loaded. Intervals
are task-clustered bootstrap, 10,000 resamples, and every row is `avg@4` over
all tasks in the domain with zero ungraded cells.

The per-trial scores behind them are not in this repository; ask us and we will
send them.

| Benchmark | Model | Bare | With M | Δ | 95% CI |
|---|---|---:|---:|---:|---|
...
The package was evolved on a mid-capability model and transferred to these
three unchanged; none of them participated in evolution. The pattern worth
noticing is that the gain tracks how much treatable failure the model still has
in that domain, not how strong the model is: the same Claude-Opus-5 buys
+15.57 on retail where it starts at 72.4, and +1.00 on airline where it already
starts at 89.5. Same model, same package, two domains — no capability
...
+2.3 points over pure retry (60.9% vs 58.6%, N=4, not significant). Most of the
headline gain over a single attempt comes from the extra attempts, not from the
learning.
...
frozen splits. All three benchmarks are third-party: we ship adapters and
pinned upstream references, not the benchmarks. Components of our research
infrastructure that produce no reported number are not part of this release.

**1. The evolution loop needs a coding agent you supply.** Every generative
step of a campaign — diagnosis, patching, candidate repair, round review — is a
call to an external agentic coding CLI through a single eight-argument launcher
...
proprietary third-party software. Substituting a different harness requires
only that it emit the same stream-json event format.
...
fork pins both to a single dated model snapshot, which is part of the
evaluation protocol rather than an implementation detail. Every τ² arm —
including the open-weight ones — therefore needs a paid endpoint.

**Absolute τ² scores in this repository are not comparable to the public
τ²-Bench leaderboard**, which uses a different judge model. Cross-arm
comparisons within this repository are valid, because both arms of every
comparison are judged by exactly the same model. This is the single most likely
thing for a reader to miss.
...
belong to their authors. Each `third_party/*/setup.sh` pins the upstream
commit; tau2's additionally verifies the fetched payload against committed
checksums, because its domain data is what every reported tau2 number was
measured against.

**4. Terminal-Bench 2.1 results depend on the container images**, as described
under D4. A user running different images gets a differently stratified set,
and their per-stratum numbers will not match ours.

**5. Compute.** One τ² retail campaign at k=4 is days of wall-clock and
thousands of model calls. SkillFlow and Terminal-Bench 2.1 require a Docker
host with sufficient concurrency.
...
**Agent trajectories.** They are the highest-value qualitative artefact and
they are still excluded, for three reasons in order of weight. The provider
...
on the assumption that it is fine. The trajectories carry benchmark task
content and solutions on held-out splits. And they carry absolute paths,
hostnames, and endpoint URLs from the machines they ran on.

**Per-trial scored results.** They carry none of that exposure, and they would
let a reader recompute the reported table without running anything. We send
them on request rather than committing them.

### Metric semantics

`Pass@k` here means "at least one of the first k trials succeeded". Some τ²
results are reported with the unbiased `pass^k` estimator, which asks whether
*every* one of k trials succeeds. These answer different questions and are not
interchangeable; the metric name is spelled out wherever it appears.
...
`recuris check-data` first. Then `docs/operations.md`, which collects the
failure modes that cost us the most time — resuming, pairing, the treatment
switches, Docker address exhaustion, image snapshots, and the coding-agent
timeout that hands a session an empty file and no failure signal.
...
- `src/recuris/skillmemory.py` — how a package becomes a running configuration.

`docs/architecture.md` is the longer version,
`docs/skill-memory-format.md` documents the package format, and
`skill_memories/README.md` explains which package to use for what.

## Licence

Apache-2.0; see `LICENSE`. Third-party components are fetched, not
